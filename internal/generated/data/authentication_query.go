...
func (aq *AuthenticationQuery) FirstID(ctx context.Context) (id types.ID, err error) {
	var ids []types.ID
...
func (aq *AuthenticationQuery) FirstIDX(ctx context.Context) types.ID {
...
func (aq *AuthenticationQuery) OnlyID(ctx context.Context) (id types.ID, err error) {
	var ids []types.ID
...
func (aq *AuthenticationQuery) OnlyIDX(ctx context.Context) types.ID {
...
func (aq *AuthenticationQuery) IDs(ctx context.Context) (ids []types.ID, err error) {
...
func (aq *AuthenticationQuery) IDsX(ctx context.Context) []types.ID {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *AuthenticationQuery) prepareQuery(ctx context.Context) error {
	...
	if authentication.Policy == nil {
		return errors.New("data: uninitialized authentication.Policy (forgotten import data/runtime?)")
	}
	if err := authentication.Policy.EvalQuery(ctx, aq); err != nil {
		return err
	}
	return nil
}
...
func (aq *AuthenticationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Authentication, error) {
	...
	if query := aq.withPerson; query != nil {
		if err := aq.loadPerson(ctx, query, nodes, nil,
			func(n *Authentication, e *Person) { n.Edges.Person = e }); err != nil {
			return nil, err
		}
	}
...
func (aq *AuthenticationQuery) loadPerson(ctx context.Context, query *PersonQuery, nodes []*Authentication, init func(*Authentication), assign func(*Authentication, *Person)) error {
	ids := make([]types.ID, 0, len(nodes))
	nodeids := make(map[types.ID][]*Authentication)
	for i := range nodes {
		fk := nodes[i].PersonID
